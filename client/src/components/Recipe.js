...
function Recipe({ user }) {
  ...
  const handleDeleteRecipe = (recipeId, index) => {
    if (!disabledButtons[index]) {
      setDisabledButtons(
        disabledButtons.map((value, i) => (i === index ? true : value))
      );
        ...
        >
          <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
            <ul class="space-y-2 font-medium">
              ...
              <li>
              <form className="flex-1 ml-3 whitespace-nowrap">
        ...
        <div className="p-4 border-2 border-dashed rounded-lg dark:border-gray-700">
          <div className="grid grid-cols-3 md:grid-cols-2 gap-4 bg-orange-100">
            {recipes.map((recipe, index) => (
              <div key={recipe.id} className="p-20 border border-black rounded-lg">
                <h5 className=" items-center w-full bg-brown shadow dark:bg-gray-800 dark:border-gray-700 font-bold text-center">
                  {recipe.name}
                </h5>
                <img
                  src={recipe.image}
                  className="flex h-auto max-w-full rounded-lg"
                  alt={recipe.name}
                />
                ...
                <Link to={`/recipedetails/${recipe.id}`} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" >
                  more details
                </Link>
                <button className="text-gray-900 border border-grey hover:border-gray-200 dark:border-gray-900 dark:bg-gray-900 dark:hover:border-gray-700 bg-white focus:ring-4 focus:outline-none focus:ring-gray-300 rounded-full text-base font-medium px-5 py-2.5 text-center mr-3 mb-3 dark:text-white dark:focus:ring-gray-800"
                  onClick={() => handleSaveRecipe(recipe.id, index)}>
                  Save
                </button>
